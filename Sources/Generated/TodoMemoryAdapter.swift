...
public class TodoMemoryAdapter: TodoAdapter {
    var items: [String:Todo] = [:]

    public func findAll(onCompletion: @escaping ([Todo], Error?) -> Void) {
        onCompletion(items.map { $1 }, nil)
    }

    public func create(_ model: Todo, onCompletion: @escaping (Todo?, Error?) -> Void) {
    ...
    public func findOne(_ maybeID: String?, onCompletion: @escaping (Todo?, Error?) -> Void) {
    ...
    public func update(_ maybeID: String?, with model: Todo, onCompletion: @escaping (Todo?, Error?) -> Void) {
    ...
    public func delete(_ maybeID: String?, onCompletion: @escaping (Todo?, Error?) -> Void) {
        guard let id = maybeID else {
            return onCompletion(nil, AdapterError.invalidId(maybeID))
        }
